...
public class GraphBasedHPFragment<NodeType, EdgeType> extends
	...
	private GraphBasedHPFragment(final Frequency freq,
			...
			final boolean storeEmbeddings,
			final GThreadEnvironment<NodeType, EdgeType> tenv,
			double ranking, ArrayList<Integer> maxRankedEdgeIds, int lastNumberOfEmbeddings) {
		super(freq, fragment, graphSet, storeEmbeddings);
		this.tenv = tenv;
		super.setRanking(ranking);
		for (int i = 0; i < maxRankedEdgeIds.size(); i++) {
			super.addMaxRankedEdgeId(maxRankedEdgeIds.get(i).intValue());
		}
		//super.setMaxRankedEdgeId(maxRankedEdgeId);
		super.setLastNumberOfEmbeddings(lastNumberOfEmbeddings);
	}
	...
	public HPFragment<NodeType, EdgeType> copy() {
		return new GraphBasedHPFragment<NodeType, EdgeType>(getFreq(),
				...
				getRanking(), getMaxRankedEdgeIds(), getLastNumberOfEmbeddings());
	}
