...
import cn.edu.pku.sei.intellide.graph.extraction.mail_to_neo4j.entity.MailInfo;
...
public class MboxHandler extends AbstractContentHandler {
    ...
    private void createUserNode(Node mailNode, String userName, String userAddress, boolean sender) {
        ...
        if (!mailUserMap.containsKey(userAddress)) {
            ...
            userNode.addLabel(MailGraphBuilder.MAILUSER);
            userNode.setProperty(MailGraphBuilder.MAILUSER_MAIL, userAddress);
        ...
        if (sender)
            mailNode.createRelationshipTo(userNode, MailGraphBuilder.MAIL_SENDER);
        else
            mailNode.createRelationshipTo(userNode, MailGraphBuilder.MAIL_RECEIVER);
    ...
    private static void createMailNode(Node node, String subject, String id, String senderName, String senderMail,
        ...
        node.addLabel(MailGraphBuilder.MAIL);
        node.setProperty(MailGraphBuilder.MAIL_SUBJECT, subject);
        node.setProperty(MailGraphBuilder.MAIL_ID, id);
        node.setProperty(MailGraphBuilder.MAIL_SENDER_NAME, senderName);
        node.setProperty(MailGraphBuilder.MAIL_SENDER_MAIL, senderMail);
        node.setProperty(MailGraphBuilder.MAIL_RECEIVER_NAMES, String.join(", ", receiverNames));
        node.setProperty(MailGraphBuilder.MAIL_RECEIVER_MAILS, String.join(", ", receiverMails));
        node.setProperty(MailGraphBuilder.MAIL_DATE, date);
        node.setProperty(MailGraphBuilder.MAIL_BODY, body);
        //node.setProperty(MailGraphBuilder.MAIL_MAIN_TEXT,MailUtil.extractMainText(body));
    }
