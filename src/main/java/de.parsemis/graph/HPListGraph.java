...
// todo konstruktor, getClassNumber
...
//	}
	...
	private HPListGraph(final String name, final int id) {
		// added
		this(name, id, DEFAULTSIZE, DEFAULTSIZE, 0);
	...
	private HPListGraph(final String name, final int id, final int nodeSize,
		...
		edge_weigths = new int[edgeSize];
		edge_rankings = new double[edgeSize];
		status = new int[MAXEDGE + 1];
	}
	
	// added
	private HPListGraph(final String name, final int id, final int nodeSize,
			final int edgeSize, int classNumber) {
		this.name = name;
		this.id = id;
		...
		if (idx < 0) {// no empty edge available
			...
			if (idx >= length) {// resize nodes
				...
				System.arraycopy(edge_labels, 0,
						...
						length);
				// added
				System.arraycopy(edge_weigths, 0,
						edge_weigths = (new int[newlength]), 0,
						length);
				System.arraycopy(edge_rankings, 0,
						edge_rankings = (new double[newlength]), 0,
						length);
			}
		} else {
	...
	@SuppressWarnings("unchecked")
	private final int _addEdge(final int na, final int nb,
			final EdgeType label, int direction, int weigth) {
		int idx = status[EMPTYEDGE];
		if (idx < 0) {// no empty edge available
			idx = status[MAXEDGE]++;
			final int length = edge_labels.length;
		...
		++status[EDGECOUNT];
		edge_labels[idx] = label;
		edge_weigths[idx] = weigth;
		edge_rankings[idx] = 0.0;
		edgesChanged();
		return idx;
	}
	...
	@Override
	public int addEdgeIndex(final int nodeAIdx, final int nodeBIdx,
			final EdgeType label, final int direction, int weigth) {
		return _addEdge(nodeAIdx, nodeBIdx, label, direction, weigth);
	}
	...
	public int addNodeAndEdgeIndex(final int nodeAIdx,
			final NodeType nodeLabel, final EdgeType edgeLabel,
			final int direction, int weigth) {
		final int nodeBIdx = addNodeIndex(nodeLabel);
		_addEdge(nodeAIdx, nodeBIdx, edgeLabel, direction, weigth);
		return nodeBIdx;
	}
	...
	public HPGraph<NodeType, EdgeType> clone() {
		...
		for (int i = this.edge_labels.length - 1; i >= 0; --i) {
			...
			// added
			ret.edge_weigths[i] = this.edge_weigths[i];// clone??
			ret.edge_rankings[i] = this.edge_rankings[i];// clone??
		}
	...
	// added 2008.09.10 (matthias)
	@Override
	public int getEdgeWeigth(final int edgeIdx) {
		return edge_weigths[edgeIdx];
	}
	
	// added
	@Override
	public void setEdgeRanking(final int edgeIdx, double r) {
		edge_rankings[edgeIdx] = r;
	}
	@Override
	public double getEdgeRanking(final int edgeIdx) {
		//System.out.println("getEdgeRanking-HPListGraph " + this.name);
		return edge_rankings[edgeIdx];
	}
