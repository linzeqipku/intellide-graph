...
public final class Settings<NodeType, EdgeType> {
	...
	private final static <NodeType, EdgeType> boolean parseOption(
		...
		if (temp[0].equals("--help")) {
			...
			//added
		} else if (temp[0].equals("--nodeRanker")) {
			if  (temp[1].equals("variance")) {
				set.nodeRanker = "variance";
			} else if (temp[1].equals("infoGain")) {
				set.nodeRanker = "infoGain";
			} else if (temp[1].equals("pearson")) {
				set.nodeRanker = "pearson";
			}
		} else if (temp[0].equals("--nodeRankerMinSize")) {
			set.nodeRankerMinSize = Integer.parseInt(temp[1]);
		} else if (temp[0].equals("--nodeRankerLowerBound")) {
			set.nodeRankerLowerBound = Double.parseDouble(temp[1]);
		} else if (temp[0].equals("--nodeRankerUpperBound")) {
			set.nodeRankerUpperBound = Double.parseDouble(temp[1]);
		} else if (temp[0].equals("--nodeRankerSubsequentBugs")) {
			set.nodeRankerSubsequentBugs=Boolean.parseBoolean(temp[1]);
		} else if (temp[0].equals("--discriminativeFrequencies")) {
			String[] weigths = temp[1].split(",");
			set.discriminativeFrequencies = new ArrayList<Integer>();
			for (int i = 0; i < weigths.length; i++) {
				set.discriminativeFrequencies.add(Integer.parseInt(weigths[i]));
			}
	...
	//added
	public String nodeRanker = null;
	public int nodeRankerMinSize = 0;
	public double nodeRankerLowerBound = 0;
	public double nodeRankerUpperBound = Double.longBitsToDouble(0x7ff0000000000000L); // positive infinity
	public boolean nodeRankerSubsequentBugs = false;
	
	public ArrayList<Integer> discriminativeFrequencies;
	...
	private Settings(final Settings set) {
		...
		//added
		this.nodeRanker = set.nodeRanker;
		this.nodeRankerMinSize = set.nodeRankerMinSize;
		this.nodeRankerLowerBound = set.nodeRankerLowerBound;
		this.nodeRankerUpperBound = set.nodeRankerUpperBound;
		this.nodeRankerSubsequentBugs = set.nodeRankerSubsequentBugs;
		this.discriminativeFrequencies = set.discriminativeFrequencies;
	}
