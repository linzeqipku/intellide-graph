...
public class ListGraph<NodeType, EdgeType> implements
	...
	private class MyEdge implements Edge<NodeType, EdgeType> {
		...
		MyEdge(final ListGraph<NodeType, EdgeType> me, final int idx,
				final int nodeA, final int nodeB, final int direction,
				final EdgeType label, final int weigth) {
			this.me = me;
			this.idx = idx;
			this.nodeA = nodeA;
			this.nodeB = nodeB;
...
//	}
	...
	//added
	private ListGraph(final String name, final int id, int classNumber) {
		this.name = name;
	...
	public Edge<NodeType, EdgeType> addEdge(
		...
		if (emptyEdge == -1) {
			idx = edges.size();
			final MyEdge ret = new MyEdge(this, idx, nodeA.getIndex(), nodeB
					.getIndex(), direction, label, weigth);
			((MyNode) nodeA).addEdge(idx, direction);
			((MyNode) nodeB).addEdge(idx, -direction);
			edges.add(idx, ret);
			edgeCount++;
	...
	public int getEdgeWeigth(final int edgeIdx) {
		return getEdge(edgeIdx).getWeigth();
	}
	
	@Override
	public double getEdgeRanking(final int edgeIdx) {
		return getEdge(edgeIdx).getRanking();
	}
	@Override
	public void setEdgeRanking(final int edgeIdx, double r) {
		MyEdge edge = this.edges.get(edgeIdx);
		edge.setRanking(r);
		this.edges.set(edgeIdx, edge);
	}
