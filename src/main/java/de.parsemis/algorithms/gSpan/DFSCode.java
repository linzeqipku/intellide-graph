...
public class DFSCode<NodeType, EdgeType> extends
	...
	public SearchLatticeNode<NodeType, EdgeType> extend(
		...
		//		nextParents);
		//added
		HPFragment<NodeType, EdgeType> tmp = ext.getFragment();
		tmp.setLastNumberOfEmbeddings(this.getLastNumberOfEmbeddings());
		ArrayList <Integer> maxRankedEdgeIds = this.getMaxRankedEdgeIds();
		maxRankedEdgeIds.trimToSize();
		tmp.setMaxRankedEdgeIds(maxRankedEdgeIds);
		tmp.setPruningMark(this.getPruningMark());
		tmp.setRanking(this.getRanking());
		return tenv().getCode(tmp, nextFirst, nextLast,
					nextParents);
	}
	...
	private final Fragment<NodeType, EdgeType> toCleanFragment() {
		...
		for (int i = old.getMaxEdgeIndex() - 1; i >= 0; --i) {
			if (old.isValidEdge(i)
				...
				// added
				int edgeIndex = ng.addEdgeIndex(nodes[old.getNodeA(i)], nodes[old.getNodeB(i)],
						old.getEdgeLabel(i), old.getDirection(i));
				ng.setEdgeRanking(edgeIndex, old.getEdgeRanking(i));
			}
		}
	...
	// added
	@Override
	public void setEdgeRanking(final int edgeIdx, double r) {me.setEdgeRanking(edgeIdx, r);}
	@Override
	public double getEdgeRanking(final int edgeIdx) {return me.getEdgeRanking(edgeIdx);}
	
	@Override
	public void setLastNumberOfEmbeddings(int size) {me.setLastNumberOfEmbeddings(size);}
	@Override
	public int getLastNumberOfEmbeddings() {return me.getLastNumberOfEmbeddings();}
	@Override
	public void addMaxRankedEdgeId(int id) {me.addMaxRankedEdgeId(id);}
	@Override
	public void setMaxRankedEdgeIds(ArrayList<Integer> ids) {me.setMaxRankedEdgeIds(ids);}
	@Override
	public ArrayList<Integer> getMaxRankedEdgeIds() {return me.getMaxRankedEdgeIds();}
	@Override
	public void setRanking(double ranking) {me.setRanking(ranking);}
	@Override
	public double getRanking() {return me.getRanking();}
	
	@Override
	public void setPruningMark(boolean flag) {me.setPruningMark(flag);}
	@Override
	public boolean getPruningMark() {return me.getPruningMark();}
